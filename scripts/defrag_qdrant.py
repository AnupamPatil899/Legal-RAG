...
print(
    f"Source Collection '{COLLECTION_NAME}': status={info.status}, points={info.points_count}, segments={info.segments_count}"
)
...
for i in range(0, len(all_points), BATCH_SIZE):
    ...
    points_to_upsert = [PointStruct(id=r.id, vector=r.vector, payload=r.payload) for r in batch]
